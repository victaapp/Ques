...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Question
        fields = ["id", "title", "description", "created", "last_modified", "ask_byy", "ask_by"]
        # fields = "__all__"

class AnswerSerializer(serializers.ModelSerializer):
    ans_byy = serializers.SerializerMethodField()
    que = serializers.SerializerMethodField()

    def get_ans_byy(self, instance):
        usr = User.objects.all().filter(id=instance.ans_by.id).last()
        ser = UserSerializer(usr)
        return ser.data
    def get_que(self, instance):
        usr = User.objects.all().filter(id=instance.question.id).last()
        ser = UserSerializer(usr)
        return ser.data

    class Meta:
        model = Answer
        fields = ["id", "question", "que", "description", "created", "last_modified", "ans_byy", "ans_by"]
        # fields = "__all__"
...
class QuesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Question
        fields = ["id", "title", "description", "created", "last_modified", "ask_by"]